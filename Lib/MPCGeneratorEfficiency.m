function lin_mat = MPCGeneratorEfficiency(MPCParams,val_LQR)
...
for t_start = 1:D
    ...
    for ind = 1:no_of_sims
        ...
        if ind<no_of_sims %Initial charging and contracting actions are all the same
    ...
    % Inequalities : p(b|s)_t (s_{t-D} + eta_p|n(bp_t - bp_{t-1}) - w_t) <= l_t
    ...
    for ind=1:no_of_sims
        ...
        A_mpc((ind-1)*4*M+1:(ind-1)*4*M+M,(ind-1)*(3*M+D+1)+1:(ind-1)*(3*M+D+1)+M) = diag(-1./prices_est((ind-1)*3+2,:)); %Buying/charge
        A_mpc((ind-1)*4*M+M+1:(ind-1)*4*M+2*M,(ind-1)*(3*M+D+1)+1:(ind-1)*(3*M+D+1)+M) = diag(-1./prices_est(ind*3,:)); %Selling/charge
        A_mpc((ind-1)*4*M+2*M+1:(ind-1)*4*M+3*M,(ind-1)*(3*M+D+1)+1:(ind-1)*(3*M+D+1)+M) = diag(-1./prices_est((ind-1)*3+2,:)); %Buying/discharge
        A_mpc((ind-1)*4*M+3*M+1:ind*4*M,(ind-1)*(3*M+D+1)+1:(ind-1)*(3*M+D+1)+M) = diag(-1./prices_est(ind*3,:)); %Selling/discharge   
        
        A_mpc((ind-1)*4*M+1:ind*4*M,(ind-1)*(3*M+D+1)+M+1:(ind-1)*(3*M+D+1)+2*M+1) = [etas(1)*[A_mpc_1;A_mpc_1] ; etas(2)*[A_mpc_1;A_mpc_1]];
        A_mpc((ind-1)*4*M+1:ind*4*M,(ind-1)*(3*M+D+1)+2*M+2:ind*(3*M+D+1)) = [A_mpc_2;A_mpc_2;A_mpc_2;A_mpc_2];
        
        A_mpc_ramping((ind-1)*2*M+1:ind*2*M,(ind-1)*(3*M+D+1)+M+1:(ind-1)*(3*M+D+1)+2*M+1) = [A_mpc_1;-A_mpc_1];
    end
    b_mpc = [b_mpc;b_mpc_ramping];
    A_mpc = [A_mpc; A_mpc_ramping];
